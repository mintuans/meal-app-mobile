...
export const Dashboard = () => {
  ...
  const budgetUsed = 750000;
  const budgetLimit = profile?.grocery_limit || 1500000;
  const percentUsed = Math.round((budgetUsed / budgetLimit) * 100);
    ...
    <div className="space-y-8 pb-12 animate-in fade-in duration-700">
      ...
      <header className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <Button variant="ghost" size="icon" className="rounded-2xl bg-white dark:bg-slate-800 shadow-sm border border-slate-100 dark:border-slate-700">
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="bg-emerald-500 p-5 rounded-[28px] text-white shadow-xl shadow-emerald-500/30 relative overflow-hidden group">
          ...
          <p className="text-2xl font-black mt-1">1,240 <span className="text-xs font-medium">kcal</span></p>
        ...
        <div className="bg-slate-900 dark:bg-white p-5 rounded-[28px] text-white dark:text-slate-900 shadow-xl shadow-slate-900/10 relative overflow-hidden">
          <p className="text-[10px] font-bold uppercase tracking-widest opacity-50">Ngân sách còn</p>
          <p className="text-2xl font-black mt-1">{formatCurrency(budgetLimit - budgetUsed)}</p>
