...
export const WeeklyPlanner = () => {
  ...
  const [formData, setFormData] = React.useState({
    ...
    kcal: '',
    tags: '',
    ingredients: [] as { id: string, name: string, amount: string, price: number }[],
    steps: [] as string[]
  });
  ...
  const handleAddIngredient = (ing: { id: string, name: string, price: number }) => {
    if (formData.ingredients.find(item => item.id === ing.id)) return;
    setFormData({
      ...formData,
      ingredients: [...formData.ingredients, { ...ing, amount: '100g' }]
    });
    setSearchIng('');
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-6 overflow-y-auto">
              ...
              <div className="space-y-4">
                ...
                <div className="relative">
                  ...
                  {showTypeDropdown && (
                    <div className="absolute z-60 top-full left-0 right-0 mt-2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl shadow-xl overflow-hidden">
                      {mealTypes.map(type => (
                        <div
                          key={type.id}
              ...
              <div className="space-y-3">
                ...
                <div className="relative">
                  ...
                  {showIngDropdown && searchIng && (
                    <div className="absolute z-50 top-full left-0 right-0 mt-2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl shadow-2xl max-h-40 overflow-y-auto">
                      {allIngredients.filter(i => i.name.toLowerCase().includes(searchIng.toLowerCase())).map(ing => (
                        <div
                          key={ing.id}
                          className="p-3 hover:bg-slate-100 dark:hover:bg-slate-700 cursor-pointer text-sm flex justify-between"
                ...
                <div className="space-y-2">
                  {formData.ingredients.map(ing => (
                    <div key={ing.id} className="flex items-center gap-3 p-2 bg-slate-50 dark:bg-slate-800/50 rounded-xl border border-slate-100 dark:border-slate-800">
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-bold truncate">{ing.name}</p>
                        <p className="text-[10px] text-slate-500">{formatCurrency(ing.price)}/đơn vị</p>
                      </div>
                      <input
                        className="w-20 p-1 text-xs border border-slate-200 dark:border-slate-700 rounded bg-transparent"
                        value={ing.amount}
                        onChange={e => {
                          const newIngs = [...formData.ingredients];
                          const target = newIngs.find(item => item.id === ing.id);
                          if (target) target.amount = e.target.value;
                          setFormData({ ...formData, ingredients: newIngs });
                        }}
                      />
                      <button type="button" onClick={() => handleRemoveIngredient(ing.id)} className="text-rose-500 p-1">
                        <Trash2 size={16} />
                      </button>
                    </div>
                  ))}
